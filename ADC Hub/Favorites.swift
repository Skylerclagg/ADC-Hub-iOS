...
struct FavoriteTeamsRow: View {
    ...
    var body: some View {
        NavigationLink(destination: TeamInfoView(teamNumber: team)
...
struct FavoriteEventsRow: View {
    ...
    var body: some View {
        NavigationLink(destination: EventView(event: (data[sku] ?? Event(sku: sku, fetch: false)))
                        .environmentObject(settings)
                        .environmentObject(dataController)) {
            VStack {
...
struct Favorites: View {
    ...
    func generate_event_sku_map() {
        DispatchQueue.global(qos: .userInteractive).async { [self] in
            ...
            let data = ADCHubAPI.robotevents_request(request_url: "/events", params: ["sku": favorites.favorite_events])
    ...
    var body: some View {
        VStack {
            Form {
                Section($favorites.favorite_teams.count > 0 ? "Favorite Teams" : "To add a Favorite Team search for them in the Lookup tab") {
                ...
                Section($favorites.favorite_events.count > 0 ? "Favorite Events" : "To add a Favorite Event search for it in the Lookup tab") {
