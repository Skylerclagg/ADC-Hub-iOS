...
public class ADCHubAPI {
    ...
    public var middle_school_world_skills_cache: WorldSkillsCache
    public var high_school_world_skills_cache: WorldSkillsCache
    public var combined_world_skills_cache: WorldSkillsCache
    ...
    public init() {
        ...
        self.combined_world_skills_cache = WorldSkillsCache()
    ...
    public static func robotevents_competition_scraper(params: [String: Any] = [:]) -> [String] {
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) { (response_data, response, error) in
            if response_data != nil {
                ...
                let regex = try! NSRegularExpression(pattern: "https://www\\.robotevents\\.com/robot-competitions/RE-[A-Z0-9]*([+-]?(?=\\.\\d|\\d)(?:\\d+)?\\.?\\d*)(?:[Ee]([+-]?\\d+))?([+-]?(?=\\.\\d|\\d)(?:\\d+)?\\.?\\d*)(?:[Ee]([+-]?\\d+))?\\.html", options: [.caseInsensitive])
                let range = NSRange(location: 0, length: html.count)
                let matches = regex.matches(in: html, options: [], range: range)
                
                for match in matches {
                    sku_array.append(String(html[Range(match.range, in: html)!]).replacingOccurrences(of: "https://www.robotevents.com/robot-competitions/ADC/", with: "").replacingOccurrences(of: ".html", with: ""))
                }
    ...
    public func generate_season_id_map() {
        ...
        // Fetch all seasons data from RoboEvents API
        let seasons_data = ADCHubAPI.robotevents_request(request_url: "/seasons/")
        
        // Loop through all season data and filter for the ADC program (Program ID = 44)
        for season_data in seasons_data {
            if let program = season_data["program"] as? [String: Any], let program_id = program["id"] as? Int, program_id == 44 {
                // Map ADC seasons to the season_id_map
        ...
        let task = URLSession.shared.dataTask(with: request as URLRequest) { (response_data, response, error) in
            if response_data != nil {
                do {
                    ...
                    completion(data)  // Pass fetched data to completion handler
                } catch {
...
public class Team: Identifiable {
    ...
    public func fetch_info() {
        ...
        let data = ADCHubAPI.robotevents_request(request_url: "/teams", params: self.id != 0 ? ["id": self.id, "program": ADCHubAPI.selected_program_id()] : ["number": self.number, "program": [44]])
    ...
    public func fetch_events(season: Int? = nil) {
        ...
        var season_id: Int
        let grade = (self.grade == "High School" || self.grade == "Middle School") ? self.grade : "High School"  // Default to "High School" if neither
        ...
        // Compare the grade with the selected grade and adjust season_id accordingly
