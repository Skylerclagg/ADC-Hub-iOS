                ...
                Section("Competition") {
                    ...
                    Picker("Competition", selection: $grade_level) {
                        ...
                        .onChange(of: grade_level) { grade in
                            ...
                            DispatchQueue.global(qos: .userInteractive).async {
                                ...
                                // Updated logic to handle different grade levels, including ADC MS/HS
                                let selectedIndex = UserSettings.getGradeLevel() == "College" ? 1 :
                                                    UserSettings.getGradeLevel() == "ADC Middle School" ? 2 :
                                                    UserSettings.getGradeLevel() == "ADC High School" ? 3 : 0

                                // Select the appropriate season based on the competition type
                                settings.setSelectedSeasonID(id: API.season_id_map[selectedIndex].keys.sorted().reversed()[0])
                                settings.updateUserDefaults(updateTopBarContentColor: false)
                    ...
                    HStack {
                        ...
                        if showLoading || API.season_id_map.isEmpty {
                            ...
                            // Adjusting season selection to account for ADC MS/HS
                            Picker("Season", selection: $selected_season_id) {
                                let selectedIndex = UserSettings.getGradeLevel() == "College" ? 1 :
                                                    UserSettings.getGradeLevel() == "ADC Middle School" ? 2 :
                                                    UserSettings.getGradeLevel() == "ADC High School" ? 3 : 0

                                ForEach(API.season_id_map[selectedIndex].keys.sorted().reversed(), id: \.self) { season_id in
                                    Text(format_season_option(raw: API.season_id_map[selectedIndex][season_id] ?? "Unknown")).tag(season_id)
                                }
                            }.labelsHidden()
